...
class Controller implements java.awt.event.ActionListener {
	...
	public void actionPerformed(java.awt.event.ActionEvent e) {
		...
		try {
			/*
			 * if(database.queryDB.checkUserType(type)==true){ ask the admin
			 * whether they want to login as a user or an admin }
			 */
			if (database.queryDB.checkUser(usn) == true
					&& database.queryDB.checkPass(database.hash.sha1(usn + pwd)) == true) {
				if (usn.equals("admin")) {
