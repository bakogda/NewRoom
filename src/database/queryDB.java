...
public class queryDB {
	...
	public static void main(String[] args) throws SQLException, ClassNotFoundException {
		...
		for (int i = 1; i <= cCount; i++)
			System.out.format("%28s", rsmd.getColumnName(i) + " | ");
	...
	public static void query(String theQuery) throws SQLException, ClassNotFoundException {
		...
		while (resultSet.next()) {
			System.out.println("");
			for (int x = 1; x <= colCount; x++)
				System.out.format("%15s", resultSet.getString(x) + " | ");
		}
	...
	public static void addEvent(String userid, String title, String date, String room, String st, String et,
			String desc) throws ClassNotFoundException, SQLException {
		String addEventSQL = "insert into " + database.JDBConnect.tbl_event
				+ "(USER_ID, TITLE, DATE, ROOM, STARTTIME, ENDTIME, DESCR) values ('" + userid + "','" + title + "','"
				+ date + "','" + room + "','" + st + "','" + et + "','" + desc + "')";
		System.out.println(addEventSQL);
		Connection connection = DriverManager.getConnection(dBV.JDBC_URL);
	...
	public static boolean histEvent(String date, String username) throws SQLException, ClassNotFoundException {
		...
		ResultSet resultSet = statement
				.executeQuery("SELECT * FROM event WHERE DATE<'" + date + "'AND USERNAME='" + username + "'");
		ResultSetMetaData rsmd = resultSet.getMetaData();
