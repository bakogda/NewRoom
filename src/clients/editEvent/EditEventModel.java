...
public class EditEventModel {
	...
	public static String getDetails(String userid) throws SQLException {
		...
		String SQL_statement = "SELECT EVENT_ID, DATE,ROOM, TO_CHAR(STARTTIME, 'HH24:MI') AS STARTTIME, TO_CHAR(ENDTIME, 'HH24:MI') AS ENDTIME, DESCR FROM EVENT WHERE USER_ID='"
				+ userid + "'AND TITLE='" + eName + "'";
		...
		while (resultSet.next()) {
			...
			String eID = resultSet.getString("EVENT_ID");
	...
	public static String getEventNames(String userid) throws SQLException {
		String SQL_statement = "SELECT EVENT_ID, TITLE, DATE, ROOM, TO_CHAR(STARTTIME, 'HH:MI'), TO_CHAR(ENDTIME, 'HH:MI'), DESCR FROM EVENT WHERE USER_ID='"
				+ userid + "' AND DATE >= NOW()";
		System.out.println(SQL_statement);
		Connection connection = DriverManager.getConnection(dBV.JDBC_URL);
	...
	public static String saveChanges(String dateEvent, String timeFr, String timeTo, String room, String descr,
			String userid, String eName) throws SQLException {
		Connection connection = DriverManager.getConnection(dBV.JDBC_URL);
		String SQL_statement = "UPDATE EVENT SET DATE='" + dateEvent + "', STARTTIME ='" + timeFr + "', ENDTIME='"
				+ timeTo + "', ROOM='" + room + "', DESCR='" + descr + "' WHERE USER_ID='" + userid + "'AND TITLE='"
				+ eName + "'";
		System.out.println(SQL_statement);
		Statement statement = connection.createStatement();
